...
//			return new Action("skip"); // TODO: improve
			...
			String lastClearDir = Direction.getDirectionFromInts(lastX, lastY).toString();
			RelativeCoordinate posToClear = mapManager.getCurrentPosition().getCoordAfterWalkingInDir(lastClearDir);
...
//		}
	...
	private Action move(String dir) {
		if (attachedBlocks.size() == 1) {
			String attachedBlockDir = getBlockDir(attachedBlocks.get(0));
			rotateAccordingToAttachedBlockUtil(dir, attachedBlockDir);
		}
		RelativeCoordinate posToWalkTo = mapManager.getCurrentPosition().getCoordAfterWalkingInDir(dir);
		if (mapManager.getObstacleLayer().containsKey(posToWalkTo)) {
			if (mapManager.getObstacleLayer().get(posToWalkTo) != null) {
				return clear(dir);
			}
		}
